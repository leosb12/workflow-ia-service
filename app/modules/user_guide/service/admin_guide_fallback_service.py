...
from app.modules.user_guide.schemas.guide_request import (
    AdminGuideRequest,
...
from app.modules.user_guide.schemas.guide_response import (
    AdminGuideIntent,
    AdminGuideResponse,
    GuideIssue,
    GuideSeverity,
...
class _KeywordSuggestion:
    responsible: str
    reason: str
    form: list[SuggestedFormField]


class AdminGuideFallbackService:
    def build_response(
        self,
        ...
        if intent == AdminGuideIntent.SUGGEST_ACTIVITY_FORM:
        ...
        if intent == AdminGuideIntent.SUGGEST_RESPONSIBLE:
        ...
        if intent in {
            AdminGuideIntent.VALIDATE_POLICY,
            AdminGuideIntent.EXPLAIN_POLICY_ERROR,
            AdminGuideIntent.HELP_ACTIVATE_POLICY,
        }:
            return self._build_validation_response(request, intent, issues, suggested_actions)

        if intent in {
            AdminGuideIntent.EXPLAIN_SCREEN,
            AdminGuideIntent.WHAT_CAN_I_DO_HERE,
            AdminGuideIntent.GUIDE_STEP_BY_STEP,
            AdminGuideIntent.HELP_CREATE_POLICY,
        }:
            return self._build_screen_response(request, intent, issues, suggested_actions)

        if intent == AdminGuideIntent.SUGGEST_DECISION:
            return self._build_decision_response(request, intent, issues, suggested_actions)

        if intent == AdminGuideIntent.SUGGEST_NEXT_ACTIVITY:
            return self._build_next_activity_response(request, intent, issues, suggested_actions)

        if intent == AdminGuideIntent.OPTIMIZE_POLICY:
    ...
    def _build_screen_response(
        ...
        if screen == GuideScreen.POLICY_DESIGNER:
            answer = (
        ...
        elif screen == GuideScreen.POLICY_LIST:
            answer = (
        ...
        if intent == AdminGuideIntent.GUIDE_STEP_BY_STEP:
    ...
    def _build_validation_response(
        ...
        if not issues:
            ...
            severity = GuideSeverity.SUCCESS
            steps = [
    ...
    def _build_general_help_answer(self, request: AdminGuideRequest) -> str:
        if request.screen == GuideScreen.POLICY_DESIGNER:
            return (
    ...
    def _build_suggested_actions(
        ...
        if request.screen == GuideScreen.POLICY_DESIGNER and "ADD_ACTIVITY" in current_actions:
            add_action("ADD_ACTIVITY", "Agregar actividad")
        if request.screen == GuideScreen.POLICY_DESIGNER and "SAVE_POLICY" in current_actions:
